...
- **This app's database is CI's relationships-index fixture.** The `index` job runs `db:init`, `db:seed:demo` and then `pnpm -F astromech-demo index:rebuild --check`, which fails on drift; against an unseeded database the check scans zero sources and passes vacuously. That script runs the BUILT CLI rather than `tsx` like the `db:*` ones, because `index:rebuild` composes domain services that import `virtual:astromech/config`, and only the tsup CLI bundle aliases it to the config shim.
