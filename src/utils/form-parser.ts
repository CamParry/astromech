...
import { ENTRY_LEVEL_FIELDS } from '@/admin/config/field-config';
 ...
 */
export function parseFormData(formData: FormData, fieldGroups: FieldGroup[]): JsonObject {
    const fields: JsonObject = {};

    // Iterate through all field definitions
    for (const group of fieldGroups) {
        for (const field of group.fields) {
            // Skip entry-level fields (they're handled separately at the entry level)
            if ((ENTRY_LEVEL_FIELDS as readonly string[]).includes(field.name)) {
                continue;
            }
            ...
            if (field.type === 'repeater') {
                    ...
                    for (let i = 0; i <= maxIndex; i++) {
                        ...
                        for (const subField of subFields) {
                            ...
                            if (
                                ...
                                item[subField.name] =
                                    subField.multiple || subField.type === 'multiselect'
                                        ? vals
                                        : (vals[0] ?? null);
                            } else {
                ...
                if (rawValue instanceof File) {
                    ...
                    fields[field.name] = parseValueFromInput(
                        rawValue,
                        field.type
                    ) as JsonValue;
                }
