...
function configWithRedirects(): AstromechConfig {
    ...
    // The redirect hook derives paths from the entry type's `url` template.
    const post = config.entries['post'];
    if (post) post.url = 'https://example.com/{slug}';
    return { ...config, plugins: [redirects()] };
}
...
describe('redirects — own-table storage', () => {
    ...
    it('stamps the qualified type onto entries read back from the own table', async () => {
        ...
        const fetched = await redirectEntriesApi().get({ type: 'redirect', id: created.id });
