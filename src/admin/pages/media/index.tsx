...
import { queryKeys } from '@/admin/hooks/useQueryKeys.js';
import { useStoredView } from '@/admin/hooks/use-stored-view.js';
...
export function MediaIndexPage(): React.ReactElement {
    ...
    const [viewMode, setViewMode] = useStoredView('media');
    ...
    const { checkedIds, toggle, toggleAll, allChecked, someChecked, reset } = useSelection(items);
    ...
    const bulkDeleteMutation = useMutation({
        ...
        onSuccess: (_data, ids) => {
            void queryClient.invalidateQueries({ queryKey: queryKeys.media.list(queryParams) });
            reset();
            toast({ message: t('media.deletedToast', { count: ids.length }), variant: 'success' });
        },
        onError: (err) => {
                ...
                <Toolbar>
                    ...
                    <ToolbarRight>
                        <ToggleGroup
                            ...
                            items={[
                                { value: 'grid', icon: <LayoutGrid size={15} />, label: t('common.gridView') },
                                { value: 'list', icon: <LayoutList size={15} />, label: t('common.listView') },
                            ]}
                        />
                    </ToolbarRight>
                </Toolbar>
                    ...
                    >
                        {isEmpty ? (
                                    ...
                                    <EmptyState
                                        ...
                                        description={
                                            q
                                                ...
                                                : t('media.noTypeResults', { type: t(`media.filter${typeFilter.charAt(0).toUpperCase() + typeFilter.slice(1)}`) }).toLowerCase()
                                        }
                                    />
                                ) : (
                                    <UploadZone
                                ...
                                <>
                                    ...
                                    <Pagination currentPage={currentPage} totalPages={totalPages} onPage={(p) => setPage(String(p))} />
                                </>
                            )}
                    </DropZone>
                )}
