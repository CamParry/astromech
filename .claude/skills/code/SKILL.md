...
- **Every repository is reached through a registry.** The repository file exports `getXRepository()`, backed by `createRegistry`, which builds the default on first use, and an `@internal` `setXRepository()` for tests. Entries keep `getEntryRepository(type)`, keyed by type, and `getEntriesTableRepository()` for the shared table's own reads. A service never builds a repository, and never passes it `config` or a `db`: the handle is resolved per call, so a transaction scope reaches it.
- **Per call on `ctx`, per site from a registry.** The user, role, access and requested locale arrive as arguments. The database, storage, email and repositories come from registries.
- **No shared resource base.** `createRepository` is a tool and `createContentRepository` holds only the three-table mechanics (join, decode, staging, versions, content-row writes). Users, media, globals and entries each write their own reads (list, count, sort, search, locale fallback) and hand-pick from the content repository, never spreading it. `owners` stays private.
- **Repository method names are fixed.** Reads: `findOne` (the row or `null`), `findMany`, `count`, `findBy<X>`, `countBy<X>`. Writes: `create`, `update`, `delete`, `upsert`, their `…Many` forms, `updateBy<X>`, `deleteBy<X>`. A resource's `findMany(params)` and `count(params)` take `{ sort, locale, limit, offset }` plus its filters; a fallback read takes `{ fallbackLocale }`. A method keeps a domain verb only when it encodes a concurrency rule (`claim`, `recordRunAndRelease`, `createIfEmpty`). Service methods keep their own names (`query`, `get`, `list…`).
...
- Repositories are **factory functions**: `createUserRepository() => ({ … })`. The one class is `TableRepository`, the pluggable `EntryRepository` implementation.
...
- **Row and mapper names.** A table's row types are `XTableRow` / `NewXTableRow`; the joined row a repository returns is `XRow`; a module-level mapper from a table row to that row is `toXRow`; a mapper from a row to the public type is `toX` (`toEntry`, `toGlobal`); a module's own table repository, inside its repository, is `owners`.
