...
- **`handleScheduled`** (`astromech/scheduler/cloudflare`, beside the driver) is the Worker's `scheduled()` entry point. It calls `ensureBooted()` first: a Cron Trigger never reaches the middleware, so this path boots the runtime itself.
