...
- **`SchedulerDriver`** (`src/cron/drivers/{interval,cloudflare,webhook}.ts`, published as `interval()`, `cloudflareCron()` and `webhook()`) abstracts _triggering only_ — it knows nothing about which jobs exist or are due.
