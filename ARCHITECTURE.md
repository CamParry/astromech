...
- **`integrations/`** holds two kinds of glue side by side. A **framework
  ...
  routes (admin shell, API, media) and the boot middleware. A **platform
  ...
  Worker entry and looks up bindings. A platform only gets a directory when it
  needs one — Node and Vercel need no code (`decisions/0089`).
...
builds the record the plugin `ctx` exposes. A platform integration declares its
