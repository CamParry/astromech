...
- **`handleScheduled`** (`src/boot/scheduled.ts`, re-exported from `astromech/scheduler/cloudflare` beside the driver) is the Worker's `scheduled()` entry point. It calls `createAstromech({ config })` first: a Cron Trigger never reaches the middleware, so this path creates the application itself. It lives in `boot/` because booting the runtime is what makes it an entrypoint — `decisions/0053-scheduled-entrypoints-live-in-boot.md`.
