...
- Populated values *replace* the ID in `fields`, so the read shape varies by call.
...
- IDs **and their order** always live in the entry's field data. No exceptions: author, category,
  single media, gallery. Array position *is* the ordering; the index is never allowed to own it
...
- The index is **rebuildable from field data**, which is what makes it safe for it to be polymorphic.
  ...
  polymorphic table being *authoritative*; a wrong index is repairable, wrong truth is data loss.
- Index rows are keyed by *(source type, source id, **field path**, target type, target id)* —
...
- **Populate must re-apply the same visibility predicates as a direct read.** Both Payload
  ...
  relation traversal. We have a public/full axis *and* staging to leak through.
...
or filtered; the index gives that to *every* field, so the historical justification is gone. A
...
*version* of B, touch its timestamp, and collide with any staged copy of B. Preferred framing if it
...
- [ ] **One-user-one-profile**: a uniqueness constraint the JSON cannot express. Leaning toward a
      write-time validation that *reads* the index, so the index stays purely derived — not a DB
      constraint *on* the index. Do not overcomplicate.
