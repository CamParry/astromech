...
- Relationships / content-field data model → `relationships-model.md`. **Note the direction changed:** the old sketch here (authoritative polymorphic relationships table, nested-in-block `instanceId`-keyed rows) is superseded — field data is the source of truth and the `relationships` table becomes a derived, rebuildable index keyed on field _path_.
