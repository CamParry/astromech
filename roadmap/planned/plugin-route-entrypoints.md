...
`virtual:` (`DECISIONS.md`).

**Status:** filed, not started. The plugin/core boundary closed the immediate
hole with a
...
one, and `DECISIONS.md` can stop treating
...
`ctx` being the single bridge is a feature worth the per-capability cost.
`DECISIONS.md`
