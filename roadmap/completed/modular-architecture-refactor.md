...
- [x] Stage 4 — extract `plugins/{seo,redirects,menus}` → separate published `@astromech/*` npm packages (decided: real monorepo, not internal workspace). Done as three vertical slices (menus → redirects → seo), each gate-green + browser-verified:
    - [x] Public plugin-authoring API (`astromech/plugin-kit` subpath) — added incrementally, only what the packages actually reach: `sanitisePackage`, `derivePluginName`, `pluginTablePrefix`, `resolveEntryUrl`, `resolveEntryPath`, `tableStorage`, `t`. (`pluginSchemaModule`/`pluginAssetRoot` not exposed — a graduated package hardcodes `{package}/schema` and `{package}` as its asset root; `labelToSlug`/`TableStorageOptions` weren't reached.)
        - **Being dissolved** by `roadmap/in-progress/plugin-authoring-experience.md` §2c. Verified not a runtime boundary — its 20 runtime-reachable modules are all pure, and root `astromech` already contains most of them. The generic exports move to the root surface and the subpath is deleted; the identity helpers leave the public API entirely.
...
- [x] Untangle the `plugins/runtime` ↔ `entries` cycle: the runtime declares an entry-access port (`plugins/runtime/entry-access.ts`, leaf-typed) and the entries domain injects the impl at boot via `wireEntryAccess()` (`entries/plugin-access.ts`, service-free, called from kernel/boot + the test harness). Added the `plugins-runtime-is-a-capability` dep-cruiser rule and folded plugins/runtime back into the acyclic `no-circular` scope.
...
- [x] Close-out (merge) — merged `refactor/modular-architecture` → `main` and pushed to origin; this file moved to `roadmap/completed/`.
