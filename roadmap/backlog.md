...
- [ ] `mediaService.replace` has no method descriptor — the only service method anywhere that lacks one (verified programmatically across users/media/settings/entries during P2). So it is absent from the method manifest and invisible to the CLI, MCP and the AI surface, and P2's fail-closed `scopedServices` refuses it even for a `*` role. Giving it one means deciding its permission (`media:upload`, presumably, matching `update`) and its input schema, which is `binaryInput` territory — likely lands with the media-ingest item above rather than on its own
