...
**Related:** [[plugin-architecture.md]], [[unified-architecture.md]], [[content-visibility.md]]. Memories: `project_content_visibility.md`, `reserved_instance_keys.md`.
...
The SDK/API muddle was a real source of confusion; these terms must stay distinct.

| Term                    | Meaning                                                                                                                                                                                                                            |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **service**             | The underlying capability — the CMS's verbs (entries, media, users, settings). Pure, permission-agnostic, self-describing. Feature-split in code; one unified layer conceptually. _Replaces the conflated meaning of "local SDK"._ |
| **operation / method**  | One verb on a service, e.g. `entries.create`, `plugins.redirects.lookup`. The unit the manifest and the AI deal in.                                                                                                                |
...
| **surface**             | A thin adapter that exposes services to a caller: `local` SDK, CLI, MCP server, the HTTP **API**. Each marshals in/out and chooses which policies to compose.                                                                      |
| **consumer**            | A _client of a surface_, not a surface itself. The `fetch` SDK is a consumer of the HTTP API (the producer/consumer pair).                                                                                                         |
...
1. **One unified services layer, feature-split in code.** Extract the capability out of `src/sdk/local` (which today conflates _the capability_ with _a surface_). `core` is dissolved over time — permissions, visibility, plugin-runtime move to their own feature directories; only genuinely-core primitives stay. Organisation is provisional and will be re-evaluated as it shakes out.
2. **Surfaces are thin adapters over services; consumers sit downstream of surfaces.** `local` SDK / CLI / MCP / HTTP API are surfaces. The `fetch` SDK is a **consumer of the HTTP API**, not a service surface. Layering: `services → (policies) → surfaces → consumers`.
3. **Policies compose over pure services.** Permissions, visibility (already `withDefaultShape`), and confirmation are middleware a surface opts into. Trusted surfaces (CLI/MCP/seeding) compose none; the HTTP API composes permissions; the AI tool-loop composes permissions **and** the confirm gate.
...
8. **Two schema worlds, both carried.** **Method schema** (Zod) = how to _call_ an operation. **Content schema** (`FieldDefinition[]` → JSON Schema, a second walk of the existing type-gen) = what to _fill_ for an entry type. Entry methods link to their type's content schema. Both already convert to JSON Schema via shipped machinery.
...
13. **The authoring AI is a single prompt-input agent with a tool-loop over the manifest**, _not_ a set of pre-declared buttons. The user says "create a page" / "add a user with this email" in chat; the agent figures out which operations to call. Delivered as an **in-built plugin** (gets `PluginContext` with `sdk`, scoped `entries`, `user`, **`env` for the API key**).
...
- **Mode A** — inline staged edits in the open dirty editor with accept/reject; needs live-buffer plumbing and a "AI edits form state" surface.
