...
- `required` declared **twice**: `FieldDefinition.required` flag _and_ a `{ required: true }` rule. Only the flag is read.
...
- Four domains, four independent unvalidated blobs. The _envelope_ diverges (title vs alt vs name vs key); the _fields-blob handling is identical and identically broken_. That uniformity is the seam for a shared primitive.
