...
- **Severity** and **document-level `validate`** are small additions *to the pipeline* —
...
Run the *declarative* rules (min/max/length/pattern/email/url/enum/required) in the
...
joins an existing locale group. The update path *pushes* non-translatable values to
...
The risk: naively mirroring update's *push* on create would, in the blank-in-group
