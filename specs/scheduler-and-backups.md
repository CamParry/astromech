...
**Status:** Designed (2026-06-15); unimplemented. Two deliverables in order: **(1) a cadence-aware, runtime-adaptive scheduler** (core; proven by tests, no dogfood plugin needed), then **(2) `@astromech/backups`** built on top.
...
**Overlap (named so we don't rebuild one inside the other):** a cron tick is a natural **producer** into a queue (nightly fan-out: "enqueue one job per tenant"); a queue's **delayed-delivery** can mimic "run once at time T" — but *recurring cadence* stays a scheduler concern. The relationship is **composition, not inheritance**.
...
`event.cron` native per-expression routing stays available as a driver-internal *optimization*, never the contract.
...
Backups is the **orchestrator** composing three seams: **when** (scheduler), **how to dump** (DB driver), **where to put it** (storage). The dump *production*, not the destination, is the hard, runtime-divergent part.
...
### Slice A — Cadence-Aware Scheduler (core; tests-only validation)
