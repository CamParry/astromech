...
Four reviews, merged: deep passes over `packages/astromech/src/policies/`
(2026-08-03, §B), `entries/` (2026-08-04, §F and §G) and `fields/` (2026-08-04,
§H, which forced the repo-wide §I and §J), plus a high-level pass over the core's
module names, public exports and concept vocabulary (2026-08-04, §A and §C–§E).

They are one branch because they keep landing on each other: §A renames a type
that §B renames a container of, and §I renames the suffix §A and §F are both
written in. Doing them apart means resolving the same conflict three times.
...
Most replacements are existing ecosystem words, but three need a
`TERMINOLOGY.md` entry (§H1 layout field, §I the `EntryType`/`Entry` asymmetry,
§J `resource`) and §I overturns an existing one. Three items warrant `decisions/`
notes (§A, §C5, §I).
...
> **Amended by §I.** `ServiceMethodDescriptor` → `ServiceMethod`: a `defineX`
> returns an `X`, and `defineServiceMethod` already exists with 21 uses. The
> `descriptors.ts` → `methods.ts` file rename in §A4 is unaffected and now doubly
> justified. Read the two rows above as `ServiceMethod` / `methods.ts`.
...
# §F — table exports take a `Table` suffix

Added 2026-08-04, from the entries review. Repo-wide, not entries-only.

> **Composes with §I.** §F suffixes the exported _const_; §I sheds `Descriptor`
> from the _type_. Together: `export const entriesTable: Table = defineTable(…)`.
> Reword this section's remaining "table descriptor" prose to "table", and
> rename `database/descriptor-snapshot.ts` → `table-snapshot.ts`.
...
Blast radius: 25 descriptor value-import lines (11 in `src/`, 14 in `tests/`),
...
### G8. Two ways to declare an entry type — consistency, not naming

Found alongside the `defineEntryType` question. Filed here because there is no
open plugin-consistency roadmap file (`plugin-consistency-sweep.md` is in
`completed/`); move it if one opens.

```ts
apps/demo/src/entries/author.ts:10              export const author = defineEntryType({ … })
plugins/forms/src/entries/form.ts:29            export const formEntryType: EntryTypeConfig = { … }
plugins/forms/src/entries/submission.ts:14      export const submissionEntryType: EntryTypeConfig = { … }
plugins/redirects/src/entries/redirect.ts:15    export const redirectEntryType: EntryTypeConfig = { … }
```

The split falls on root-config vs plugin, but nothing forces it: `redirect.ts`
already imports `tableStorage` from `astromech`, so the helper costs no new
import.

`apps/docs/content/entry-types.md:97-103` justifies the helper as "checking at
the point of the mistake" — but that argument is against an _unannotated_ plain
object, and the annotation form satisfies it equally. `defineEntryType` is a
non-generic identity function, so the two are type-equivalent. This is style, and
the reason to settle it is that one pattern should be discoverable rather than
two.

Adopt `defineEntryType` in the three plugin files: it is what the docs
demonstrate, what `index.ts:155` demonstrates, and what the ecosystem ships
(`defineConfig`, Sanity's `defineType`). Then say so in the entry-types guide, so
plugin authors reading it aren't shown one pattern and served another by the
first-party plugins.
...
**§A.** The per-method metadata row becomes `ServiceMethod`, not
`ServiceMethodDescriptor`. §A4's file rename (`<domain>/descriptors.ts` →
`methods.ts`) survives unchanged and is now doubly justified: the files hold
`ServiceMethod` objects.

**§F.** Its export-suffix decision is unaffected and composes cleanly:
`export const entriesTable: Table = defineTable(...)`. The const keeps the
`Table` suffix (§F), the type sheds `Descriptor` (§I). §F's prose needs rewording
from "table descriptor" to "table", including `database/descriptor-snapshot.ts` →
`table-snapshot.ts`.
...
- **`defineEntryType`** stays. Considered `defineEntry` for symmetry with
  `defineTable` / `defineConfig` / `defineAdminPage`, and rejected: it returns
  `EntryTypeConfig`, and an entry is an _instance_ of an entry type, a
  distinction `TERMINOLOGY.md` §"Entry type vs EntryTypeConfig" already draws.
  `defineTable` gets away with the short form because a table has its own noun
  for the schema-level thing; entries don't (Payload uses "collection";
  Astromech deliberately doesn't), so "entry type" is the only name it has and
  dropping "Type" makes the helper claim to define a single entry. Sanity's
  `defineType` is the matching prior art.
...
- [ ] `apps/docs/content/entry-types.md` — for §G8, state that
      `defineEntryType` is the way to declare one, and replace the
      "checking at the point of the mistake" rationale, which argues against an
      unannotated object rather than against `: EntryTypeConfig`.
