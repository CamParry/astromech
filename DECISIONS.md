...
**A repository answers exact reads; fallback is the service's.** `get` on the users and media repositories returns the row asked for, or null; the fallback chain lives in `findUser` and `findMedia`. A one-off account question uses the table repository (`users.owners.count({ role: 'admin' })`). Rejected: a named repository method per question, which hides read policy behind a name.
