...
- **Query keys come from `hooks/use-query-keys.ts`, and writes from a resource module's `xMutations()` table** (`hooks/entries.ts`, `globals.ts`, `media.ts`, `users.ts`, `notifications.ts`, `admin-resources.ts`), run with `useAdminMutation`. A new write is a row in its table naming the keys it invalidates.
