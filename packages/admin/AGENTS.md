...
- **A page takes an entry type or global id** and reads it through `useAdminEntryType` or `useAdminGlobal`; the route files only map params. The edit pages compose `useEditController` with the pieces in `components/entries/entry-form-fields.tsx` and `staging-controls.tsx`, and the entries list composes `useListController`.
