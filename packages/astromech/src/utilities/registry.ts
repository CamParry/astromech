 ...
 * entry chunks and a module-scoped variable is duplicated once per chunk, so a
 * driver set during Astro's config:setup hook (integration context) would be
 * invisible to the server at request time (Vite SSR context) even though both
 * run in the same process. The global is the only reliable shared slot.
 *
