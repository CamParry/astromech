...
async function runRule(rule: ValidationRule, ctx: FieldValidationContext): Promise<string | null> {
    const { value } = ctx;

    if ('minLength' in rule) {
        if ((typeof value === 'string' || Array.isArray(value)) && value.length < rule.minLength) {
    ...
    if ('maxLength' in rule) {
        if ((typeof value === 'string' || Array.isArray(value)) && value.length > rule.maxLength) {
...
export async function processFields(
    ...
    for (const field of fields) {
        ...
        let v = descriptor?.coerce ? descriptor.coerce(values[field.name]) : values[field.name];
