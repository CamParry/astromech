    ...
    return {
        ...
        entries: {
            ...
            post: {
                ...
                fields: [
                    // Required text field
                    { name: 'title_text', type: 'text', label: 'Title Text', required: true },
                    // Email field (descriptor-level validate)
                    { name: 'contact_email', type: 'email', label: 'Contact Email' },
                    // Text field with unique constraint
                    { name: 'code', type: 'text', label: 'Code', validation: [{ unique: true }] },
                    // Text field with defaultValue
                    { name: 'status_label', type: 'text', label: 'Status Label', defaultValue: 'pending' },
...
describe('create — email validation', () => {
    it('rejects an invalid email value', async () => {
        await expect(
            ...
            name: 'ValidationError',
            fields: { contact_email: expect.arrayContaining(['Must be a valid email address']) },
        });
    });
