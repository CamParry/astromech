...
function field(def: Partial<FieldDefinition> & { name: string; type: string }): FieldDefinition {
...
describe('required', () => {
    ...
    it('create + absent value with field.defaultValue → default applied, required passes', async () => {
        const { values, errors } = await processFields(
            {},
            [field({ name: 'status', type: 'text', required: true, defaultValue: 'draft' })],
...
describe('rule: minLength', () => {
    ...
    it('pass: array meets min length', async () => {
        const { errors } = await processFields(
            { tags: ['a', 'b', 'c'] },
            [field({ name: 'tags', type: 'multiselect', validation: [{ minLength: 2 }] })],
            fakeCtx(),
        );
...
describe('rule: pattern', () => {
    it('pass: string matches pattern', async () => {
        const { errors } = await processFields(
            { code: 'ABC123' },
            [field({ name: 'code', type: 'text', validation: [{ pattern: '^[A-Z]+\\d+$' }] })],
            fakeCtx(),
        );
        ...
        const { errors } = await processFields(
            ...
            [
                field({
                    name: 'code',
                    type: 'text',
                    validation: [{ pattern: '^[A-Z]+\\d+$', message: 'Use uppercase letters then digits' }],
                }),
            ],
            fakeCtx(),
        );
...
describe('rule: enum', () => {
    it('pass: value in enum', async () => {
        const { errors } = await processFields(
            { status: 'draft' },
            [field({ name: 'status', type: 'text', validation: [{ enum: ['draft', 'published'] }] })],
            fakeCtx(),
        );
        expect(errors.status).toBeUndefined();
    });
        ...
        const { errors } = await processFields(
            ...
            [
                field({
                    name: 'code',
                    type: 'text',
                    validation: [{ minLength: 5 }, { pattern: '^[A-Z]+$', message: 'Must be uppercase' }],
                }),
            ],
            fakeCtx(),
        );
...
describe('rule: custom', () => {
    it('custom returns true → no error', async () => {
        const { errors } = await processFields(
            { value: 'ok' },
            [field({ name: 'value', type: 'text', validation: [{ custom: async () => true }] })],
            fakeCtx(),
        );
                    ...
                    validation: [
                        {
                            custom: async (ctx: FieldValidationContext) => {
                                ...
                                return ctx.value === ctx.values.password || 'Passwords do not match';
                            },
                        },
                    ],
                }),
            ...
            [
                ...
                field({
                    ...
                    validation: [
                        {
                            custom: async (ctx: FieldValidationContext) =>
                                ctx.value === ctx.values.password || 'Passwords do not match',
                        },
                    ],
                }),
            ],
                ...
                field({
                    ...
                    fields: [
                        field({
                            name: 'inner',
                            type: 'section',
                            fields: [field({ name: 'deep', type: 'text', required: true })],
                        }),
                    ],
                }),
            ],
    ...
    it('all-valid input → errors is {}', async () => {
        const { errors } = await processFields(
            ...
            [
                ...
                field({ name: 'count', type: 'number', validation: [{ min: 1 }, { max: 10 }] }),
            ],
            fakeCtx(),
        );
