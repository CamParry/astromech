...
- **There are no internal barrels.** Every import names the file that declares the symbol; only `src/exports/` re-exports, and only for a published subpath (`DECISIONS.md`). A lint rule rejects an import of a path ending in `index`, bar the modules that hold real code. `src/types/index.ts` is the exception, type-only and erased. Browser code deep-imports a pure leaf; a module file the browser needs takes the `*.shared.ts` suffix, which limits what it may import in turn.
