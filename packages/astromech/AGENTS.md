...
- **The import graph is a DAG and it is enforced.** Imports point down only: entrypoints → transport → policies → domains → capabilities → leaves; peers inside a layer may read each other. `ARCHITECTURE.md` has the layer model; `pnpm run lint:deps` fails the build on a violation. The rules generate from the `LAYERS` table at the top of `packages/astromech/.dependency-cruiser.cjs`, so a new top-level `src/` directory has to be added there before the scan will pass.
