...
- [0088](0088-get-throws-resolve-may-not-and-require-is-middleware.md) — `get*` throws when the thing is absent, `resolve*` may return `undefined`, `assert*` returns void, and `require*` is reserved for middleware
- [0089](0089-cloudflare-is-a-runtime-integration-and-the-environment-has-one-reader.md) — `integrations/` holds framework and platform integrations side by side, a platform earns a directory only when its environment or entry point is non-standard, no runtime key goes in the config, and every environment read goes through `src/env/`
