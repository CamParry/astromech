...
- [0091](0091-cloudflare-is-a-runtime-integration-and-the-environment-has-one-reader.md) — `integrations/` holds framework and runtime integrations side by side, a runtime earns a directory only when its environment or entry point is non-standard, and every environment read goes through `src/env/`; whether a runtime should be declared is deferred
- [0092](0092-encoded-is-the-column-form-of-a-value.md) — the column form of a value is `encoded` (`column` and Drizzle's `driverParam` lose), the declared SQL column type is `columnType`, and `storage` in `database/` goes so the word keeps meaning blobs alone
- [0093](0093-barrels-are-entry-points-not-navigation.md) — a re-export barrel belongs at a published entry point only; the seventeen internal `index.ts` barrels go and every import names the defining file, with `src/types/index.ts` kept as a type-only exception and `sideEffects` scoped to the modules that have them
- [0094](0094-core-tests-share-a-module-graph.md) — core's vitest run drops per-file isolation and moves to the `threads` pool, with `tests/_support/isolated-tests.ts` naming the files that opt back in and a test that fails when that list drifts; the `deps.optimizer.ssr` and one-root-workspace alternatives were measured and lost
