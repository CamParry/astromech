...
- A **runtime integration** answers where environment values come from and
...
framework integrations coming and one runtime integration, and a directory
level that separates a group of five from a group of one earns nothing.

**A runtime earns a directory only when it has a non-standard environment
...
## No runtime key, for now, and not because the drivers imply one

This record adds no `runtime` key, because nothing here needs one: the Worker
entry supplies the environment as an argument, and it nominates the scheduler
as it does so.

It is worth being explicit about an argument this record does **not** make.
...
`{ binding }` or `{ bucket }`, so D1 or R2 over HTTP from Node is supported,
and Workers with `libsql()` and `s3()` names no Cloudflare driver at all. No
code infers a runtime from a driver, and none should.

Whether a runtime should be declared, inherited from the framework's adapter, or
detected the way Hono detects it, is left open in
`roadmap/planned/runtime-declaration.md`. A site on `@astrojs/cloudflare`
declares nothing extra today: it adds one entry file if it wants Cron Triggers,
...
- **A `runtime` key in `AstromechConfig`, in this record.** Deferred rather than
  rejected: nothing here needs one, and the question of whether anything should
  is its own decision, with the Hono shape as a live alternative.
- **Nesting `integrations/frameworks/` and `integrations/runtimes/`.** A
  directory level separating five from one.
- **A symmetric `RuntimeIntegration` interface with a directory per runtime.**
...
- **Keeping `bindings.ts` at the top level to avoid a driver importing from
  ...
  depending on Cloudflare's runtime module is honest rather than inverted.
