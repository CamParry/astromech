...
- [x] **Permissions overhaul** — `resource:identifier:action` grammar, segment-wise wildcards, owner-first plugin permission trees, `definePermissionBundles` + `builtInRole()`, secure-by-default plugin data — grammar flipped to action-last, `*` matches one segment / trailing `*` the remainder via a shared matcher; `definePermissionBundles` + `builtInRole` for composing roles; `suggestedRoleGrants` and the `permissionNamespace` manifest override removed (namespace is always the sanitised package)
- [x] **Abstract entry core** — universal contract (`id`/`fields`/timestamps), capabilities (`statuses`/`slug`/`translatable`/`versioning`/`trash`) with boot validation, `titleField: string | false`, `EntryStorage` interface (built-in storage = first implementation) — capabilities resolve at boot (statuses/slug/trash default on, crash-loud storage-mismatch validation); `sdk/local/entries.ts` is a storage-dispatching orchestrator over `BuiltInEntryStorage`; routes 409 `capability_not_supported` on opted-out ops and DELETE hard-deletes when trash is off; `titleField` restricted to `'title' | false` until tableStorage; in-memory libsql test harness + frozen characterization suite guard the layer; demo `snippet` type is the titleless/statuses-off testbed (remove when Phase 3 redirects validates)
- [x] **Namespaced plugin entries** — qualified identity `{plugin}/{type}`, entries API mounted per plugin namespace, auto-exposed wire + typed SDK, `ctx.entries` scoping, `tableStorage()`, app-owned `db:generate` orchestration for plugin schemas; **redirects moves to its own table** (validating plugin) — plugin entries resolve into `ResolvedConfig.pluginEntries` (root `config.entries` is user-only); `createEntriesRouter` factory mounts the same handler stack at `/api/plugins/{name}/entries/*` with perms `plugin:{ns}:entry:{type}:{action}`; `tableStorage(table)` maps non-reserved columns to fields (supports `[]`, per-type `search: [fields]` config); `astromech db:generate` code-gens a combined schema module (plugin `schemaModule` re-exports) and shells drizzle-kit; admin entry pages extracted to shared `EntriesSurface`-parameterized components serving both root routes and `/admin/plugin/{name}/entries/{type}`; typegen emits `AstromechPluginEntryTypes` powering typed `sdk.plugins.{name}.entries.*`; entry-storage registry state moved to globalThis (multi-entry-point chunk duplication had split the registry — caught by demo browser-verify); demo `snippet` testbed removed (redirects is the titleless validator)
- [x] **Definition-driven admin** — `TableDefinition`/`FormDefinition` derived client-side from `AdminEntryTypeConfig`, core entry pages refit to render through them (dogfooding rule) — `deriveTableDefinition`/`deriveFormDefinition` are pure functions over the serialized admin config; columns carry a small separate **cell-kind** vocabulary (text/title/badge/slug/date/boolean/number/relationship/locale/translations) defaulted from field type and overridable per `adminColumn.kind`; two client-side registries (cell-kind→cell component, field-type→input component) resolve the string keys the definitions carry, replacing the hand-written `String(field)` column loops + the 24-case field `switch`; the list page keeps its hand-written shell (toolbar/filters/bulk-actions/view-toggle/context-menus) and consumes the derived `TableDefinition`, new/edit pages consume the derived `FormDefinition`; titleless types (redirects) now render boolean columns (`enabled`) as ✓/— instead of literal `true`/`false`; plugin lazy-field path preserved through the registry's default branch. `PageDefinition` + a declarative page DSL deliberately deferred (no concrete demand); media/users page convergence deferred (demand-driven) — registries built generic for later adoption
- [x] **Builders + `define*` family** — field builders (`text('from').required().searchable()`, class-based, emit plain FieldDefinitions, object literals still work) + flat `fields` entry-type shortcut (resolves to a default main group; `.searchable()` collected into `search`); `defineHook(event, handler)` with event-inferred payloads flips `hooks` record → array (multiple handlers per event); `defineSdkMethod<I,O>` + plugin self-augmentation of `AstromechPluginSdks` (type generator no longer emits SDK lines; `PluginSdkNamespace` makes typed methods win over the untyped fallback); redirects dogfoods all of it (builders + flat fields + typed `lookup`); `column()`/`table()`/`form()`/`page()` builders deferred as demand-driven, matching the Phase 4 PageDefinition call.
    - **Builder half redesigned** (see `specs/entry-schema-authoring.md`, locked 2026-06-14): the chained class-based field builders were ripped out for pure **POJO object-settings factories** (`text('from', { required: true })`); `fieldGroups` → a single recursive `fields` tree (`FieldDefinition[]` or `{ main, sidebar }`) with layout containers (`section`/`tabs`/`tab`/`accordion`, flat data) as field types; `priority`/`placement` removed (array order governs); factories moved to the new `astromech/fields` subpath; admin labels became i18n-ready via `t(key)` descriptors + a `resolveLabel`/`useEntryNamespace` seam (activation deferred). The chaining-order TS limitation this bullet's API carried is therefore moot.
